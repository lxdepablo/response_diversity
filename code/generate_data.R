...
#setwd(dirname(rstudioapi::getActiveDocumentContext()$path)) # use to run locally
#setwd("../")
...
n_simulations <- 1000 # set to 1000 for full results
...
p_contribute_gaussian_ranges <- list(function_intercept_range, function_slope_range, c(0, 50), b_range, c(0, 20), sd_range)
p_contribute_linear_ranges <- list(function_intercept_range, function_slope_range, c(-100, 100), abundance_slope_range, sd_range)
...
p_contribute_gaussian_n10_results <- do.call(rbind, lapply(proportion_contribute, function(p){
...
p_contribute_gaussian_n50_results <- do.call(rbind, lapply(proportion_contribute, function(p){
  # run simulations for current value of p
...
p_contribute_linear_n10_results <- do.call(rbind, lapply(proportion_contribute, function(p){
  # run simulations for current value of p
  ...
  print("post resilience calc")
  print(colnames(curr_p))
  
  # calc weighted response diversity
  mean_weighted_response_diversity <- curr_p %>%
    group_by(sim_number, species_ID) %>%
    summarize(mean_abundance = mean(abundance), abundance_slope = median(abundance_slope), abundance_intercept = median(abundance_intercept)) %>%
    group_by(sim_number) %>%
    summarize(mean_weighted_response_diversity = Hmisc::wtd.var(abundance_slope, mean_abundance))
...
p_contribute_linear_n50_results <- do.call(rbind, lapply(proportion_contribute, function(p){
  # run simulations for current value of p
...
# write data to CSV's
write_csv(p_contribute_gaussian_n10_results, "data/sim_data/p_contribute_gaussian_n10.csv")
write_csv(p_contribute_gaussian_n50_results, "data/sim_data/p_contribute_gaussian_n50.csv")
write_csv(p_contribute_linear_n10_results, "data/sim_data/p_contribute_linear_n10.csv")
write_csv(p_contribute_linear_n50_results, "data/sim_data/p_contribute_linear_n50.csv")
